...
public:
	vector<int> numMovesStones(int a, int b, int c) {
		...
		int minMoves = 2, maxMoves = (numbers[1] - numbers[0] - 1) + (numbers[2] - numbers[1] - 1);
		if (numbers[1] - numbers[0] <= 2 || numbers[2] - numbers[1] <= 2)
			minMoves = 1;

		return { min(minMoves, maxMoves), maxMoves };
	}
};
...
int main() {
	...
	auto result1 = solution.numMovesStones(1, 2, 5); //[1, 2]
	auto result2 = solution.numMovesStones(4, 3, 2); //[0, 0]
	auto result3 = solution.numMovesStones(3, 5, 1); //[1, 2]
