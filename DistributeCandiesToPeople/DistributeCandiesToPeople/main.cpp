...
public:
	vector<int> distributeCandies(int candies, int num_people) {
		...
		int candyGiveCount = 1;
		for (int i = 0; candies > 0; ++i) {
			int index = i % num_people;

			if (candies - candyGiveCount > -1)
				answer[index] += candyGiveCount;
			else {
...
int main() {
	...
	auto answer1 = solution.distributeCandies(7, 4);
	auto answer2 = solution.distributeCandies(10, 3);
