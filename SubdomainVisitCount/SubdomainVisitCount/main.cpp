...
public:
	vector<string> subdomainVisits(vector<string>& cpdomains) {
		...
		unordered_map<string, int> subdomainTotals;

		for (auto& s : cpdomains) {
			vector<string> temp = split(s, ' ');
			vector<string> tempSub = split(temp[1], '.');
			
			int visits = stoi(temp[0]);
			string tempDomainName = "";
			for (int i = 0; i < (int)tempSub.size(); ++i) {
				tempDomainName += tempSub[i];
				for (int j = i + 1; j < (int)tempSub.size(); ++j) {
					tempDomainName += '.';
					tempDomainName += tempSub[j];
				}
				subdomainTotals[tempDomainName] += visits;
				tempDomainName = "";
			}
		...
		for (auto& p : subdomainTotals) {
			string temp = "";
			temp += to_string(p.second);
			temp += ' ';
			temp += p.first;
			result.push_back(temp);
		}
...
int main() {
	...
	vector<string> cpdomains1{ "9001 discuss.leetcode.com" };
	auto result1 = solution.subdomainVisits(cpdomains1);

	vector<string> cpdomains2{ "900 google.mail.com", "50 yahoo.com", "1 intel.mail.com", "5 wiki.org" };
	auto result2 = solution.subdomainVisits(cpdomains2);
